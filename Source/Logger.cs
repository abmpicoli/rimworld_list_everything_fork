...
namespace List_Everything
{
	...
	private static object _logEnabled = null;
		...
		private static bool LogEnabled
		{
			get
			{
				if (_logEnabled == null)
				{
					string debugEnableFile = GetRootDirectory() + Path.DirectorySeparatorChar + "ListEverything.DebugMode";
					...
					if (!(bool)_logEnabled )
					{
						...
						{
							onlyOnce = true;
							RootLogger.fatal(()=>"Debug mode is turned off. Create a file named " + debugEnableFile + " to enable it, if you want to debug this code");
						}
	...
	private static string GetRootDirectory()
	{
	  ...
	  try
	  {
		directory = GenFilePaths.SaveDataFolderPath + Path.DirectorySeparatorChar + "ListEverythingLogs";
	  }
	  catch (Exception ex)
	  {
	...
	public void VerseMessage(Func<object> v)
	{
			try
			{
				Verse.Log.Message("ListEverything." + context + ":" + v.Invoke());
			} catch
		  {
